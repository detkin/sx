...
func (h *MCPHandler) ScanInstalled(targetBase string) ([]InstalledArtifactInfo, error) {
	...
	dirs, err := os.ReadDir(mcpPath)
	if err != nil {
		return nil, fmt.Errorf("failed to read mcp-servers directory: %w", err)
	}
	...
	for _, dir := range dirs {
		...
		metaPath := filepath.Join(mcpPath, dir.Name(), "metadata.toml")
		...
		artifacts = append(artifacts, InstalledArtifactInfo{
			...
			InstallPath: filepath.Join("mcp-servers", dir.Name()),
		})
	}
